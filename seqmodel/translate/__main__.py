...
def load_api_params(tf_config, graph, api_config="../config/code2nl.yaml"):
    ...
    with open('../config/call.yaml') as f:
...
#        config.score_functions = evaluation.name_mapping[config.score_function]
    ...
    config_path = os.path.join(config.model_dir, 'call.yaml')
    if args.train and not os.path.exists(config_path):
        shutil.copy('../config/call.yaml', config_path)

    logging_level = logging.DEBUG if args.verbose else logging.INFO
    # always log to stdout in decoding and eval modes (to avoid overwriting precious train logs)
    print(config)
    print('----->',config.model_dir,config.log_file)
